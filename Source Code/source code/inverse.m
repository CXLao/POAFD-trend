function [G_recovery,n]=inverse(an,coef,t,varargin)
% Inverse algorithm
%
% G_recovery=inverse(an,coef,t)
% G_recovery=inverse(an,coef,t,'level',max_level)
% G_recovery=inverse(an,coef,t,'energy',max_energy)
%
% Descriptions:
%   (1) G_recovery=inverse(an,coef,t): Build the reconstructed signal
%       G_reconvery using all an and coef.
%   (2) G_recovery=inverse(an,coef,t,'level',max_level): Build the
%       reconstructed signal G_recovery only using first max_level an and coef.
%   (3) G_recovery=inverse(an,coef,t,'energy',max_energy): Build the
