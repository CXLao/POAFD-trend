...
% Read signal G
load('DATA.mat')
% Decompose and reconstruct signal G using the algorithm based on the conventional exhaustive
% searching
disp('Running conventional exhaustive searching')
tic;
[an_conv,coef_conv,t]=conv(G,50,50);
t_cost=toc;
disp(['Time Cost: ' num2str(t_cost)])
G_re_conv=real(inverse(an_conv,coef_conv,t));
figure(1)
plot(t,G,t,G_re_conv)
legend({'Original Signal','Conventional Exhaustive Searching'})
% Decompose and reconstruct signal G using the algorithm based on the FFT
disp('Running FFT based exhaustive searching')
tic;
[an_FFT,coef_FFT,t]=FFT(G,50,50);
t_cost=toc;
disp(['Time Cost: ' num2str(t_cost)])
G_re_FFT=real(inverse(an_FFT,coef_FFT,t));
figure(2)
plot(t,G,t,G_re_FFT)
legend({'Original Signal','FFT-based Exhaustive Searching'})
% Decomposition components
[~,~,F_n]=comp(an_FFT,coef_FFT,t);
figure(3)
for n=1:6
    subplot(3,2,n)
    plot(t,real(F_n(n+1,:)));
